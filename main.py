...
def login(user, password):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if res1.status_code == 200:
                res1_json = res1.json()
                if "access" in res1_json:
                    print("Access Code 获取成功！")
                    code = res1_json["access"]
                    break  # 成功获取，跳出循环
                else:
                    print(f"------ 用户名或密码错误，请检查后再试 ------")
                    return None, None
            elif res1.status_code == 429:
                print(f"------ 尝试 {attempt + 1}/{max_retries}: 请求过于频繁 ------")
                if attempt < max_retries - 1:
                    wait_time = base_delay * (2 ** attempt) + random.uniform(0, 5)
                    print(f"将在 {wait_time:.2f} 秒后重试...")
                    time.sleep(wait_time)
                else:
                    print("------ 已达到最大重试次数，放弃登录。 ------")
                    return None, None
            else:
                print(f"------ Access Code 获取失败，状态码: {res1.status_code}，响应: {res1.text} ------")
                return None, None
        except requests.exceptions.RequestException as e:
            print(f"------ 尝试 {attempt + 1}/{max_retries} 时发生网络错误: {e} ------")
            if attempt >= max_retries - 1:
                return None, None

    if not code:
        print("------ 最终未能获取 Access Code，登录失败。 ------")
        return None, None

    url2 = "https://account.huami.com/v2/client/login"
    if is_phone:
        data2 = { "app_name": "com.xiaomi.hm.health", "app_version": "4.6.0", "code": f"{code}", "country_code": "CN", "device_id": "2C8B4939-0CCD-4E94-8CBA-CB8EA6E613A1", "device_model": "phone", "grant_type": "access_token", "third_name": "huami_phone" }
    else:
    ...
    try:
        ...
        print("Login Token 和 UserID 获取成功！")
...
def main(_user, _passwd, min_1, max_1):
    ...
    print("==========================================")
    print(f"开始为用户 {user} 修改步数...")
    print(f"已设置为随机步数 ({min_1}~{max_1})，本次目标：{step} 步")
    ...
    finddate = re.compile(r"date%22%3A%22(.*?)%22")
    findstep = re.compile(r"ttl%5C%22%3A(.*?)%2C%5C%22dis")
    
    # 使用 re.sub 替换找到的第一个匹配项
    # 注意：原始正则表达式中的 findall 返回列表，sub 需要字符串模式
    # 我们用 re.search 来找到第一个匹配
    date_pattern = finddate.search(data_json).group(1)
    step_pattern = findstep.search(data_json).group(1)
    
    data_json = data_json.replace(date_pattern, today)
    data_json = data_json.replace(step_pattern, step)

    url = f'https://api-mifit-cn.huami.com/v1/data/band_data.json?&t={t}'
    head = {
        "apptoken": app_token,
        "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8",
        "User-Agent": "MiFit/6.9.1 (iPhone; iOS 16.6; Scale/3.00)" # 更新 User-Agent
    }
...
if __name__ == "__main__":
    if len(sys.argv) < 6:
        print("参数不足。用法: python script.name.py <users> <passwords> <use_weather> <area> <push_token>")
        sys.exit(1)
    getBeijinTime()
